...
IntTab *create_IntTab(int *content_tab) {
    ...
    content_tab = (int*) malloc(sizeof(int) * capacity);
...
void addToIntTab(IntTab *intTab, int number) {
    intTab->content_tab[intTab->size] = number;
    printf("ité = %d - content = %d\n",intTab->size, intTab->content_tab[intTab->size]);
    intTab->size++;
    if (intTab->size >= intTab->capacity) {
        intTab->capacity *= 2;
        intTab->content_tab = realloc(intTab->content_tab, sizeof(int) * intTab->capacity);
    }
}